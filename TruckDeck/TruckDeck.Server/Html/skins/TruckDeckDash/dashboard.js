/*
    "Big Rig Cluster" skin for ETS2 / ATS.
    Display-only race-style telemetry cluster. All DOM updates happen in
    filter() (called on every telemetry push) so the dashboard keeps updating
    even if the core render loop hiccups. Uses js- prefixed classes so the
    core's automatic class->value binding never overwrites our values.
*/
...
Funbit.Ets.Telemetry.Dashboard.prototype.initialize = function (skinConfig, utils) {
                ...
                fetch(url, { method: 'POST' })['catch'](function () {
                    if (!utils._bridgeWarned) {
                        ...
                        console.info('[Input Bridge] not running on :' + BRIDGE_PORT
                            + ' — in-game key commands disabled. Run Html\\input_bridge\\start_bridge.bat');
                    }
                });
            } catch (e) {
    ...
    // ---- Colour scheme: accent colour and day/night mode are independent,
    //      each cycled by its own bottom-bar button. ----
    this.accents = [
    ...
    // Migrate the old combined 'rc.theme' value (e.g. "night-blue").
    var legacy = localStorage.getItem('rc.theme');
    if (legacy && (!savedMode || !savedAccent)) {
        var parts = legacy.split('-');
        if (!savedMode) savedMode = parts[0];
        if (!savedAccent) savedAccent = parts[1];
    }
    ...
    function applyTheme() {
        ...
        var root = document.querySelector('.dashboard.tdd');
        if (root) {
            root.setAttribute('data-mode', self.mode);
            root.setAttribute('data-accent', acc[0]);
        }
        $('.js-theme-label').text(acc[1]);   // no-op when the label is hidden (dot only)
    ...
    // ---- Screen manager (local display only — not synced to game) ----
    this._cycleLock = false;
    var screenCount = $('.js-screen').length;

    function loadScreenIndex() {
        var idx = parseInt(localStorage.getItem('tdd.screenIdx') || '', 10);
        if (!isNaN(idx) && idx >= 0 && idx < screenCount) return idx;
        // Migrate legacy brand + sub-screen keys.
        var brand = localStorage.getItem('tdd.brand') || 'generic';
        var sub = parseInt(localStorage.getItem('tdd.screen') || '0', 10) || 0;
        var legacy = {
            generic: 0, man: sub === 1 ? 2 : 1, scania: sub === 1 ? 4 : 3,
            volvo: sub === 1 ? 6 : 5, daf: 7, mercedes: 8
        };
        return legacy[brand] != null ? legacy[brand] : 0;
    }
    ...
    function applyScreen() {
        ...
        var title = $screen.data('title') || ('SCREEN ' + (idx + 1));
        $('.js-screen-label').text(title);
        localStorage.setItem('tdd.screenIdx', '' + idx);
    }
    applyScreen();
    ...
    // Physical joystick button (via Input Bridge dashboard.screenCycleJoy, e.g. joy6.b69).
    ...
    // ---- Auto blinker cancel ----
    this.autoBlink = localStorage.getItem('tdd.autoBlink') !== '0';   // default ON
    ...
    (function restoreLive() {
        try {
            var saved = JSON.parse(localStorage.getItem('tdd.econLive') || 'null');
    ...
    // Hotkey Ctrl + ' (quote) -> Radio Star / Favorite
    $(document).on('keydown', function (e) {
        if (e.ctrlKey && (e.key === "'" || e.keyCode === 222)) {
            e.preventDefault();
            // Trigger the star command if it exists (porting from TCD later)
            if (typeof utils.triggerGameAction === 'function') {
    ...
    // Speed unit (km/h <-> mph), saved on this device. Tap the speed gauge.
    ...
    // Gear display style (tap the RPM/gear gauge to cycle), saved on device.
    //   num   -> 1, 2, 3, 4 ...
    //   auto  -> A1, A2, A3 ...
    //   range -> CL, CH, 1L, 1H, 2L, 2H ... (crawler + low/high split)
    this.gearModes = ['num', 'auto', 'range'];
    this.gearMode = localStorage.getItem('tdd.gearMode') || 'num';
    ...
    $(document).on('click', '.gauge-rpm', function () {
        ...
        localStorage.setItem('tdd.gearMode', self.gearMode);
    });

    // ---- Tell-tale dashboard-light icons ----
    // Inject a real dashboard-style SVG glyph into each tell-tale span (keeps
    // dashboard.html and mock.html in sync since both load this file). The
    // icon inherits the tell's colour via currentColor.
    (function () {
        var S = function (body) {
        ...
        var icons = {
            ...
            rpm:     S('<path d="M12 3v2M5.2 6.2l1.4 1.4M18.8 6.2l-1.4 1.4"/><circle cx="12" cy="13" r="7"/><path d="M12 13l3.2-3.8"/>'),
        ...
        for (var k in icons) {
            ...
            var $e = $('.js-tell-' + k);
            if ($e.length) { $e.attr('title', $e.text()); $e.html(icons[k]); }
        }
        $('.js-fuel-icon').html(icons.fuel);
        $('.js-rpm-icon').html(icons.rpm);
    })();

    // Keep the screen awake while the dashboard is shown.
    this.wakeLock = null;
    ...
    // Country name + flag (position lookup via /scripts/country-lookup.js).
    this._countriesEts2 = null;
    this._countryKey = '';
    this._countryPos = { x: null, z: null };
    function loadCountryScript() {
        if (window.RcCountry) {
            return window.RcCountry.loadEts2Countries().then(function (list) {
...
// Resolve a dotted path; returns undefined if any step is missing.
...
Funbit.Ets.Telemetry.Dashboard.prototype._saveEconLive = function () {
    var ec = this.econ;
    try {
        if (!ec.active) { localStorage.removeItem('tdd.econLive'); return; }
        localStorage.setItem('tdd.econLive', JSON.stringify({
...
Funbit.Ets.Telemetry.Dashboard.prototype._clearEconLive = function () {
    try { localStorage.removeItem('tdd.econLive'); } catch (e) { /* ignore */ }
};
...
Funbit.Ets.Telemetry.Dashboard.prototype.filter = function (data, utils) {
    ...
    // Set a CSS custom property natively (jQuery 2.x .css() can't set CSS vars).
    // De-duped: only write (and therefore repaint the big conic-gradient rings)
    // when the value actually changed, so steady speed/RPM don't thrash paint.
    ...
    // ---------- Speed (unit set by tapping the speed gauge) ----------
    ...
    $('.js-speed').text(shownSpeed);
    $('.js-speed-unit').text(isMph ? 'MPH' : 'KM/H');
    var speedCol = '#ffffff';
    if (speedZone === 'over') speedCol = '#ff453a';
    else if (speedZone === 'warn') speedCol = '#ffb020';
    else if (speedZone === 'ok') speedCol = '#39e07a';
    ...
    // Race-style 0..25000 scale (the truck's rev range mapped onto 25k).
    var rpmShown = Math.round(rpmFrac / 100 * 25000);
    $('.js-rpm').text(rpmShown);
    // Ring colour: green to 13k, amber to 18k, red to 20k, white past 20k.
    var rpmCol;
    if (rpmShown >= 20000) rpmCol = '#ffffff';
    else if (rpmShown >= 18000) rpmCol = '#ff453a';
    else if (rpmShown >= 13000) rpmCol = '#ffb020';
    else rpmCol = '#39e07a';
    // Set on the root so BOTH the RPM ring and the top RPM bar share the colour.
    setVar('.dashboard.tdd', '--rpmcol', rpmCol);
    $('.js-rpm-icon').css('color', rpmCol);

    // ---------- Gear (tap the RPM gauge to cycle display style) ----------
    var g = truck.gear;
    if (g == null && typeof truck.displayedGear === 'number') g = truck.displayedGear;
    var gearText;
    if (g == null || g === 0) {
        gearText = 'N';
    } else if (g < 0) {
        gearText = 'R';
    } else if (self.gearMode === 'auto') {
        gearText = 'A' + g;
    } else if (self.gearMode === 'range') {
        // Pair each gear into a low/high split; first pair is the crawler.
    ...
    // Auto / manual indicator inside the gauge: use the game's shifter type
    // when it is exposed, otherwise infer it from the chosen display style.
    ...
    // ---------- RPM bars (0 - 25000) ----------
    var $rpmUnits = $('.js-rpm-bars .rpm-unit');
    if ($rpmUnits.length) {
        var rpmBarsOn = Math.ceil(rpmFrac / 100 * 25);
        // Keep a small sliver showing at idle while the engine turns.
        if (rpm > 0 && rpmBarsOn < 1) rpmBarsOn = 1;
        
        $rpmUnits.each(function(i) {
            var $u = $(this);
            $u.toggleClass('on', i < rpmBarsOn);
            // Colors: 0-12 green (up to 13k), 13-17 amber (up to 18k), 18-19 red (up to 20k), 20-24 white
            $u.removeClass('green amber red white');
            if (i < 13) $u.addClass('green');
            else if (i < 18) $u.addClass('amber');
            else if (i < 20) $u.addClass('red');
            else $u.addClass('white');
        });
    }
    ...
    $('.js-cruise').toggleClass('active', ccOn)
        .text(ccOn ? ('' + ccSpeed) : 'CRUISE OFF');
    ...
    $('.stat[data-k="oil"]').toggleClass('alert', oil != null && oil >= 120)
        .toggleClass('caution', oil != null && oil >= 105 && oil < 120);
    $('.stat[data-k="water"]').toggleClass('alert', water != null && water >= 105)
        .toggleClass('caution', water != null && water >= 95 && water < 105);
    ...
    // ---------- Odometer ----------
    var odo = num(['truck.odometer']);
    $('.js-odometer').text(odo == null ? '--' : Math.round(odo));
    ...
    if ($units.length) {
        ...
        $units.each(function(i) {
            ...
            // Fixed colors: 0-2 red, 3-5 amber, 6-9 green
            $u.removeClass('red amber green');
            if (i < 3) $u.addClass('red');
            else if (i < 6) $u.addClass('amber');
            else $u.addClass('green');
        });
        var $fuelIcon = $('.js-fuel-icon');
        $fuelIcon.removeClass('low warn ok');
        if (pct != null) {
            if (pct <= 30) $fuelIcon.addClass('low');
            else if (pct <= 60) $fuelIcon.addClass('warn');
            else $fuelIcon.addClass('ok');
        }
    }

    // Range = fuel / average consumption (l/km), shown in the active unit.
    var distUnitLabel = isMph ? 'MI' : 'KM';
    $('.js-range-unit').text(distUnitLabel);
    ...
    // ---------- Damage ----------
    var wears = [];
    ['truck.wearEngine', 'truck.wearTransmission', 'truck.wearCabin',
     'truck.wearChassis', 'truck.wearWheels'].forEach(function (p) {
        var v = self._rv(data, p);
        if (typeof v === 'number' && isFinite(v)) wears.push(v);
        
        // Update individual wear readouts
        var key = p.split('.').pop(); // wearEngine, etc.
        $('.js-' + key).text(typeof v === 'number' ? Math.round(v * 100) : '--');
    });
    var dmg = null;
    if (wears.length) dmg = Math.max.apply(null, wears);
    else dmg = num(['truck.wearSum']);
    if (dmg != null) {
        var dpct = Math.round(dmg * 100);
        $('.js-damage').text(dpct);
        $('.stat[data-k="dmg"]').toggleClass('alert', dpct >= 50)
            .toggleClass('caution', dpct >= 20 && dpct < 50);
    } else {
        $('.js-damage').text('--');
    }
    ...
    var steerRot = steer * 120;                 // visual wheel rotation
    $('.js-steer').css('transform', 'rotate(' + steerRot.toFixed(0) + 'deg)');
    $('.js-steer-deg').text(Math.abs(Math.round(steerRot)));
    $('.js-steer-dir').text(Math.abs(steer) < 0.02 ? '' : (steer < 0 ? 'L' : 'R'));

    // ---------- Inclination / declination (pitch) ----------
    // Telemetry orientation is in revolutions (1.0 = 360 degrees).
    ...
    while (pitchDeg < -180) pitchDeg += 360;
    var tiltRot = Math.max(-50, Math.min(50, -pitchDeg * 3)); // exaggerate for visibility
    $('.js-tilt').css('transform', 'rotate(' + tiltRot.toFixed(1) + 'deg)');
    $('.js-incl-deg').text(Math.abs(pitchDeg).toFixed(1));
    $('.js-incl-arrow').text(Math.abs(pitchDeg) < 0.3 ? '\u2192' : (pitchDeg > 0 ? '\u2191' : '\u2193'));
    ...
    // ---------- Dashboard tell-tale lights ----------
    var setTell = function (cls, on) { $('.js-tell-' + cls).toggleClass('on', !!on); };

    // Treat the blinker as on if either the lamp ("active") or the switch
    // ("on") field is true, so the side glow stays lit while the CSS animation
    // does the actual blinking (the server may not toggle the boolean per flash).
    var blinkL = bool(['truck.blinkerLeftActive']) || bool(['truck.blinkerLeftOn']);
    var blinkR = bool(['truck.blinkerRightActive']) || bool(['truck.blinkerRightOn']);
    ...
    // Turn-signal side overlays flash with the blinker state.
    ...
    $('.js-dist-unit').text(distUnitLabel);
    ...
    // Time-to-go: how long the trip still takes (the estimated travel duration).
    if (isFinite(navMs) && navMs > 0) {
        var ttgMin = Math.round(navMs / 60000);
        var ttgH = Math.floor(ttgMin / 60);
        var ttgM = ttgMin % 60;
        $('.js-ttg').text(ttgH > 0 ? ttgH + 'h ' + pad(ttgM) + 'm' : ttgM + 'm');
    } else {
        $('.js-ttg').text('--');
    }

    // Job deadline: how much time is left to finish the job.
    ...
    if (isFinite(jobMs) && jobMs > 0) {
        ...
        // Fallback to deadlineTime if remainingTime is not available or zero
    ...
    // ---------- Current country (GPS nearest-country lookup) ----------
    ...
    if (window.RcCountry) {
        ...
        if (isFinite(posX) && isFinite(posZ)) {
            motion = {
                prevX: self._countryPos.x,
                prevZ: self._countryPos.z
            };
...
Funbit.Ets.Telemetry.Dashboard.prototype.render = function (data, utils) {
    // All rendering is handled in filter() for robustness.
};
