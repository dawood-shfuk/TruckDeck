/*
    TruckDeck Mercedes — digital cluster skin.
    Feature parity with TruckDeckDash; Mercedes digital layout.
*/

Funbit.Ets.Telemetry.Dashboard.prototype.initialize = function (skinConfig, utils) {
    var self = this;
    var BRIDGE_PORT = 25556;
    var SKIN = '.dashboard.mercedes-digital';
    ...
    var legacyTheme = localStorage.getItem('rc.theme');
    if (legacyTheme && (!savedMode || !savedAccent)) {
        var parts = legacyTheme.split('-');
    ...
    for (var ai = 0; ai < this.accents.length; ai++) {
        if (this.accents[ai][0] === (savedAccent || 'ice')) { this.accentIdx = ai; break; }
    }
    ...
    this.autoBlink = localStorage.getItem('tdd.autoBlink') !== '0';
...
Funbit.Ets.Telemetry.Dashboard.prototype._saveEconomySession = function () {
    ...
    try {
        var log = JSON.parse(localStorage.getItem('tdd.fuelLog') || '[]');
        ...
        localStorage.setItem('tdd.fuelLog', JSON.stringify(log));
    } catch (e) { /* ignore */ }
};
...
Funbit.Ets.Telemetry.Dashboard.prototype.filter = function (data, utils) {
    ...
    var barProp = self.barHorizontal ? 'width' : 'height';
    $('.js-speed-bar').css(barProp, speedPct + '%')
    ...
    var $rpmBar = $(SKIN + ' .merc-rpm-bar .js-rpm-bar');
    var rpmPortrait = window.matchMedia('(max-aspect-ratio: 1/1)').matches;
    if (rpmPortrait) {
        $rpmBar.css({ height: '100%', width: rpmFrac + '%' });
    } else {
        $rpmBar.css({ width: '100%', height: rpmFrac + '%' });
    }
    $rpmBar.removeClass('zone-green zone-amber zone-red zone-white')
    ...
    $('.js-gearmode, .js-drive-mode, .js-emode').text(isAuto ? 'AUTO' : 'MAN');
    ...
    if (adbluePct != null) {
        $('.js-adblue-bar').css('width', adbluePct + '%');
    }
