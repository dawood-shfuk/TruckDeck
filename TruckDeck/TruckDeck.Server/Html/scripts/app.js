        ...
        (function (Telemetry) {
            var App = (function () {
                ...
                App.prototype.initializeViewport = function () {
                    ...
                    if ($viewport.length) {
                        $viewport.attr('content', 'width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover');
                    }

                    // Dynamic skins fill the device viewport (mobile / PWA / iframe).
                    if (this.skinConfig.width < 1 || this.skinConfig.height < 1) {
                        $('body').addClass('tdd-host-dynamic');
                        var applyDynamicViewport = function () {
                            var h = window.innerHeight;
                            var w = window.innerWidth;
                            if (h > 0) {
                                $('html').css('height', h + 'px');
                                $('body').css({ height: h + 'px', minHeight: h + 'px', maxHeight: h + 'px' });
                            }
                            if (w > 0) {
                                $('html').css('width', w + 'px');
                                $('body').css('width', w + 'px');
                        ...
                        applyDynamicViewport();
                        $(window).on('orientationchange resize', function () {
                            clearTimeout(_this.resizeTimer);
                            _this.resizeTimer = setTimeout(applyDynamicViewport, 100);
                        });
                        return;
                    }
                ...
                App.prototype.initializeDashboard = function () {
                    ...
                    $.ajax({
                        ...
                        _this.dashboard = new Funbit.Ets.Telemetry.Dashboard(Telemetry.Configuration.getTelemetryUrl('/api/ets2/telemetry'), _this.skinConfig);
                    }).fail(function () {
