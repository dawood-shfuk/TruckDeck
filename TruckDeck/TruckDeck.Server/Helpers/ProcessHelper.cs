...
namespace Funbit.Ets.Telemetry.Server.Helpers
{
    public static class ProcessHelper
    {
            ...
            Encoding outputEncoding = null)
        {
            ...
            if (outputEncoding != null)
                return RunAndWaitEncoded(out output, out error, info, timeoutInSeconds);
        ...
        /// <summary>wsl.exe emits UTF-16; async line reads corrupt it — use synchronous ReadToEnd.</summary>
        static int RunAndWaitEncoded(out string output, out string error, ProcessStartInfo info, int timeoutInSeconds)
        {
            using (var process = Process.Start(info))
            {
                if (process == null || process.Handle == IntPtr.Zero)
                ...
                if (!process.WaitForExit(timeoutInSeconds * 1000))
                {
                    ...
                    throw new TimeoutException("Process timed out: " + info.FileName + " " + info.Arguments);
                }

                System.Threading.Tasks.Task.WaitAll(new[] { outputTask, errorTask }, TimeSpan.FromSeconds(5));
                output = outputTask.IsCompleted ? outputTask.Result : "";
                error = errorTask.IsCompleted ? errorTask.Result : "";
                return process.ExitCode;
            }
        }
