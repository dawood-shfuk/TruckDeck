...
namespace Funbit.Ets.Telemetry.Server
{
    public partial class SetupForm : Form
    {
        ...
        public SetupForm()
        {
            ...
            truckDeckHeader.VersionText = AssemblyHelper.Version;
            footerLabel.Text = Program.UninstallMode ? @"Removing TruckDeck components" : @"Preparing your cabin telemetry stack";

            DialogResult = DialogResult.OK;

            string port = ConfigurationManager.AppSettings["Port"];
            string ports = HttpPortSetupHelper.PortListLabel;
            if (Program.UninstallMode)
            {
                pluginStatusLabel.Text = @"Uninstall TruckSim GPS telemetry plugin";
                firewallStatusLabel.Text = $@"Delete firewall rules for ports {ports}";
                urlReservationStatusLabel.Text = $@"Delete ACL rules for http://+:{ports}/";
                okButton.Text = @"Uninstall";
            }
            else
            {
                pluginStatusLabel.Text = @"Install TruckSim GPS telemetry plugin";
                firewallStatusLabel.Text = $@"Add firewall rules for ports {ports}";
                urlReservationStatusLabel.Text = $@"Add ACL rules for http://+:{ports}/";
